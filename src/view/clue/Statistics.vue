  ...
  <div class="clue_statistics">
    	...
    	<template>
	 			<el-table :data="tableData" border stripe highlight-current-row style="width: 100%;" v-loading="listLoading">
		 				...
		 				<el-table-column prop="clueTotal" label="线索数量" align="center"></el-table-column>
		 				<el-table-column prop="disposeClue" label="已处理数量" align="center"></el-table-column>
		 				<el-table-column prop="undisposedClue" label="未处理数量" align="center" ></el-table-column>
	 		...
	 		<el-col class="pageContainer" style="margin:10px 0 0 0">
			  ...
			  <!--图表分析-->
				 <el-dialog title="图表" :visible.sync="echartDialog">
				 			<el-row>
				 					<el-col :span="24" class="bg-purple" align="center" >{{echartDialogTitle}}线索上报处理统计</el-col>
...
import { getClueStatisticsTable } from 'api'	
import echarts from 'echarts'
export default {
  name: 'clue_statistics',
  data () {
    return {
  ...
  methods:{
  	getUsers(){
  		...
  		getClueStatisticsTable(para).then((res) =>{
                  ...
                  series:[  //图形配置
                  	{
                  		...
                  		data:[ //数据
                  			{name: '已处理', value:row.disposeClue },
                  			{name: '未处理', value:row.clueTotal - row.disposeClue }
                  		]
                  	}
                  ],
