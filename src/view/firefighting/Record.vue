  ...
  <div class="firefighting_record">
    	...
    	<template>
	 			<el-table :data="tableData" border stripe highlight-current-row style="width: 100%;" v-loading="listLoading">
		 				...
		 				<el-table-column prop="uploadName" label="上传人" align="center"></el-table-column>
		 				<el-table-column prop="dateTime" label="上传时间" align="center" :formatter="formatterTableDateTime"></el-table-column>
			 ...
			 <!--查看-->
			<el-dialog title="自查内容" :visible.sync="checkContent">
		 			<el-row>
		 					<el-col :span="6" class="bg-purple" v-for="(item,index) in checkDataList" :key="index" style="position:relative;">
		 							...
		 							<template v-else>
		 								<h3>{{item.cybercafeSite}}</h3>
		 								<img src="../../assets/post01.jpg" style="width:100%;height:100px;"/>
...
import { getFirefightingRecordTable ,getFirefightingRecordTableDelete , getFirefightingRecordTableCheck , uploadImg }	from 'api'
import VVideo from '@/base/video/video'
export default {
  name: 'firefighting_record',
  data () {
    return {
  ...
  methods:{
  	...
  	getUsers(){
  		...
  		getFirefightingRecordTable(para).then((res) =>{
  				console.log(res)
...
//		this.getUsers()
		...
		handleEdit(index,row){  //查看
  		...
  		let para = {
			...
			getFirefightingRecordTableCheck(para).then((res) =>{
			...
			this.$confirm('确定要删除该记录吗？','提示',{
				...
				type : 'warning'
		  		...
		  		let para = {
					...
					getFirefightingRecordTableDelete(para).then((res) =>{
							console.log(res)
							if(res.data.code == '1'){
									this.$message({
										message: '删除成功',
